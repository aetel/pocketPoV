 ...
 *
 * Sleep code by syntec
 * http://forum.arduino.cc/index.php?topic=271534.0
 *
 * Persistence of View display code by vishalapr
...
/*
 * Default values for turning display are:
 *
 * int delayTime = 1;
 * int charBreak = 2.1;
 *
 * for long exposure pictures (a.k.a. "lightpainting") use:
 *
 * int delayTime = 50;
 * int charBreak = 110;
 */
...
void displayChar(char c)
{
  ...
  if (c == '¡'){for (int i = 0; i <5; i++){displayLine(iexcl[i]);delay(delayTime);}displayLine(0);}
  ...
  if (c == '·'){for (int i = 0; i <5; i++){displayLine(mdot[i]);delay(delayTime);}displayLine(0);}
  delay(charBreak);
}
...
void loop(){
  contador = 0;
  pinMode(LED4, OUTPUT);
  while(contador < (1500/delayTime)){
      displayString("teleco campus sur·");
      contador++;
  }
